...
void TextChat::ProcessTextChatMsg(int nTO)
{
	...
	else
	{
		...
		else
		{
			...
			// SRCACIC: Envia o texto para os logs do srcacic.                 "cli"
			CACIC_Auth::getInstance()->sendChatText(m_szRemoteText, "cli");
...
void TextChat::SendLocalText(void)
{
	...
	// SRCACIC: Envia o texto para os logs do srcacic.               "srv"
	CACIC_Auth::getInstance()->sendChatText(m_szLocalText, "srv");
