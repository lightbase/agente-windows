...
vncServer::vncServer()
{
	...
	m_clientmap = new vncClient*[MAX_VNC_CLIENTS];
	...
	m_autoportselect = FALSE; // SRCACIC: Porta passada como parâmetro.
	...
	m_loopback_allowed = TRUE; // aceita conexoes locais
	m_lock_on_exit = 0;
	// m_connect_pri =
	// 0 - disconecta os outros clientes
	// 1 - aceita a conexao
	// 2 - rejeita a conexao
	// 3 - rejeita toda conexoes
	m_connect_pri = 1; // prioridade de conexao
	m_disableTrayIcon = FALSE;
	m_AllowEditClients = TRUE; // permite que o servidor remova um cliente, por exemplo
	...
	for (int x=0; x<MAX_VNC_CLIENTS; x++)
		m_clientmap[x] = NULL;
	m_nextid = 0;
					 ...
					 rfbProtocolVersionMsg *protocolMsg)
{
	...
	do
	{
		clientid = (clientid+1) % MAX_VNC_CLIENTS;
		
		// verifica se ja iterou sobre todos possiveis slots(e não encontrou um livre)
		// nesse caso a conexão é encerrada
		if (i++ >= MAX_VNC_CLIENTS)
		{
			delete socket;
			return -1;
		}
...
{
	if ((clientid >= 0) && (clientid < MAX_VNC_CLIENTS))
...
// TightVNC 1.2.7
void
vncServer::SetPort(const UINT port_rfb)
{
	if (m_port != port_rfb) {
...
vncServer::SockConnect(BOOL On)
{
	...
	vnclog.Print(20, VNCLOG("SockConnect %d"), On);
	if (On)
	{
		...
		if (m_socketConn == NULL)
		{
			...
			if (m_autoportselect)
			{
				...
				for (int i=0; i < 99; i++)
				{
					...
					vnclog.Print(LL_CLIENTS, VNCLOG("Tentando usar a porta %d..."), m_port);
					...
					if (tempsock.Create())
					{
						if (!tempsock.Connect("localhost", m_port))
						{
							...
							{
								MessageBox(NULL, "OPA9", "", 0);
								ok = TRUE;
								break;
							}
						}
					}
				MessageBox(NULL, "OPA10", "", 0);
				}
			...
			vnclog.Print(20, VNCLOG("SockConnect  Done %d"), On);
		}
	}
	else
	{
