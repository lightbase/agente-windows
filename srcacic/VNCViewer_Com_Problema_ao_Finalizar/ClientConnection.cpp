...
void ClientConnection::Authenticate()
{
	...
	fprintf(arq, "nm_usuario_cli_enc > %s\n", CACIC_Crypt::decodifica(nm_usuario_cli_enc));
	...
	fprintf(arq, "te_senha_cli_enc > %s\n", CACIC_Crypt::decodifica(te_senha_cli_enc));
	...
	fprintf(arq, "te_node_address_cli_enc > %s\n", CACIC_Crypt::decodifica(te_node_address_cli_enc));
	...
	fprintf(arq, " te_so_cli_enc > %s\n", CACIC_Crypt::decodifica(te_so_cli_enc));
		fclose(arq);
		arq = NULL;
...
ClientConnection::CloseWindows()
{
	...
	MessageBox(m_hwndcn,"Caiu a conexao!!!","Queda na rede!!",MB_ICONERROR | MB_OK);}
	SetEvent(KillEvent);
	if (m_hwndMain) {SendMessage(m_hwndMain, WM_CLOSE, 0, 1);	
	MessageBox(m_hwndcn,"Fechou a janela!!!","Conexao caiu!!!!",MB_ICONERROR | MB_OK);}
}
ClientConnection::~ClientConnection()
{
	if (m_hwndStatus)
				...
				case WM_CLOSE:
					{
                        ...
                        if (lParam == 0)
						{
							MessageBox(hwnd,"Fechou o Cliente!!!","Fechou mesmo???",MB_ICONERROR | MB_OK);
						...
						{
							MessageBox(hwnd,"Caiu a conexao!!!","Queda na rede!!",MB_ICONERROR | MB_OK);
