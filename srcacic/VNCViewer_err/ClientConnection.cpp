...
void ClientConnection::Run()
{
	...
	*/
	...
	// desabilita a captura do clipboard, para que não interfira na aunteticação.
	this->m_opts.m_DisableClipboard = true;
	...
	// desabilita a captura do clipboard, para que não interfira na aunteticação.
	this->m_opts.m_DisableClipboard = false;
...
//	SendMessage(m_hwnd,WM_CREATE,0,0);
    ...
    AppendMenu(hsysmenu, MF_SEPARATOR, NULL, NULL);
	AppendMenu(hsysmenu, MF_STRING, IDD_ABOUT,		sz_L39);
...
void ClientConnection::NegotiateProtocolVersion()
{
	...
	*/
	...
	// CACIC: valores alterados para somente aceitar conexões do srcaciccli
	if (m_minorVersion == 17)
	{
		m_ms_logon = true;
		m_fServerKnowsFileTransfer = true;
	}
	if (m_minorVersion == 19) // 6 because 5 already used in TightVNC viewer for some reason
	{
		m_ms_logon = false;
		m_fServerKnowsFileTransfer = true;
	}
...
void ClientConnection::Authenticate()
{
	...
	char nm_usuario_cli_enc[48];
	memset(nm_usuario_cli_enc, 0, sizeof(nm_usuario_cli_enc));
	strcpy(nm_usuario_cli_enc, CACIC_Crypt::codifica(ad.m_user).c_str());

	char te_senha_cli_enc[48];
	memset(te_senha_cli_enc, 0, sizeof(te_senha_cli_enc));
	strcpy(te_senha_cli_enc, CACIC_Crypt::codifica(ad.m_passwd).c_str());

	char te_node_address_cli_enc[48];
	memset(te_node_address_cli_enc, 0, sizeof(te_node_address_cli_enc));
	strcpy(te_node_address_cli_enc, CACIC_Crypt::codifica(CACIC_Utils::getMACAddress().c_str()).c_str());

	char te_documento_referencial_enc[192];
	memset(te_documento_referencial_enc, 0, sizeof(te_documento_referencial_enc));
	strcpy(te_documento_referencial_enc, CACIC_Crypt::codifica(ad.m_doc_ref).c_str());

	char te_motivo_conexao_enc[7200];
	memset(te_motivo_conexao_enc, 0, sizeof(te_motivo_conexao_enc));
	strcpy(te_motivo_conexao_enc, CACIC_Crypt::codifica(ad.m_motivo).c_str());

	char te_so_cli_enc[48];
	memset(te_so_cli_enc, 0, sizeof(te_so_cli_enc));
	strcpy(te_so_cli_enc, CACIC_Crypt::codifica(CACIC_Utils::getSOID().c_str()).c_str());

	//* Lose the plain-text data from memory */
	memset(ad.m_user, 0, sizeof(ad.m_user));
	memset(ad.m_passwd, 0, sizeof(ad.m_passwd));

	WriteExact((char*) nm_usuario_cli_enc, sizeof(nm_usuario_cli_enc));
	WriteExact((char*) te_senha_cli_enc, sizeof(te_senha_cli_enc));
	WriteExact((char*) te_node_address_cli_enc, sizeof(te_node_address_cli_enc));
	WriteExact((char*) te_documento_referencial_enc, sizeof(te_documento_referencial_enc));

	WriteExact((char*) te_motivo_conexao_enc, sizeof(te_motivo_conexao_enc));

	WriteExact((char*) te_so_cli_enc, sizeof(te_so_cli_enc));
		...
		try
		{
			while (!m_bKillThread) 
			{
				...
				case rfbResizeFrameBuffer:
				{
					...
					FILE *arq0;
					arq0 = fopen("D:/Cacic 2.6 -20100406/update.txt","a");
					SendAppropriateFramebufferUpdateRequest();
					fprintf(arq0,"linha 3530 \n");
					fclose(arq0);
					SizeWindow();
...
{
	FILE *arq;
	arq = fopen ("D:/Cacic 2.6 -20100406/contandotrue.txt","a");
...
{
	FILE *arq;
	arq = fopen ("D:/Cacic 2.6 -20100406/contandofalse.txt","a");
    SendFramebufferUpdateRequest(0, 0, m_si.framebufferWidth,
...
void ClientConnection::SendAppropriateFramebufferUpdateRequest()
{
	if (m_pendingFormatChange) 
	{
		...
		{
			FILE *arq;
			arq = fopen ("D:/Cacic 2.6 -20100406/ddb.txt","a");
			SendIncrementalFramebufferUpdateRequest();
			fprintf(arq,"3696 true \n");
			fclose(arq);
...
void ClientConnection::ReadNewFBSize(rfbFramebufferUpdateRectHeader *pfburh)
{
	...
	m_pendingFormatChange = true;
		FILE *arq1;
		arq1 = fopen("D:/Cacic 2.6 -20100406/update.txt","a");
		SendAppropriateFramebufferUpdateRequest();
		fprintf(arq1,"linha 4666 \n");
		fclose(arq1);
	SizeWindow();
	InvalidateRect(m_hwndcn, NULL, TRUE);
	RealiseFullScreenMode();
					...
					*/	
					...
					case ID_FILETRANSFER: 
						...
						if (_this->m_pFileTransfer->m_fVisible || _this->m_pFileTransfer->m_fOldFTProtocole){
								FILE *arq2;
								arq2 = fopen("D:/Cacic 2.6 -20100406/update.txt","a");
								_this->SendAppropriateFramebufferUpdateRequest();
								fprintf(arq2,"linha 5446 \n");
								fclose(arq2);
							//_this->SendAppropriateFramebufferUpdateRequest();
						}
						return 0;
...
LRESULT CALLBACK ClientConnection::WndProchwnd(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	if (_this == NULL){
		FILE *arq11;
		arq11 = fopen("D:/Cacic 2.6 -20100406/imsg.txt","a");
		fprintf(arq11,"if %d %x\n",iMsg,iMsg);
		fclose(arq11);
		_this->SendAppropriateFramebufferUpdateRequest();
		return DefWindowProc(hwnd, iMsg, wParam, lParam);
	}
_this->SendAppropriateFramebufferUpdateRequest();
	FILE *arq10;
	arq10 = fopen("D:/Cacic 2.6 -20100406/imsg.txt","a");
	fprintf(arq10,"no if%d \n",iMsg);
	fclose(arq10);
	...
	switch (iMsg) 
			...
			case WM_REGIONUPDATED:
				//_this->DoBlit();
					FILE *arq3;
					arq3 = fopen("D:/Cacic 2.6 -20100406/update.txt","a");
					_this->SendAppropriateFramebufferUpdateRequest();
					fprintf(arq3,"linha 6270 ** %d  %x\n", iMsg,iMsg);
					fclose(arq3);
				//_this->SendAppropriateFramebufferUpdateRequest();
				return 0;
