...
uses Windows, Forms, IniFiles, SysUtils, Classes, Registry, MSI_GUI;
var  p_path_cacic, p_path_cacic_ini : string;

type
  Tfrm_col_undi = class(TForm)
...
function Tfrm_col_undi.SetValorChaveRegIni(p_Secao: String; p_Chave: String; p_Valor: String; p_Path : String): String;
var Reg_Ini     : TIniFile;
begin
    FileSetAttr (p_Path,0);
    Reg_Ini := TIniFile.Create(p_Path);
//    Reg_Ini.WriteString(frm_col_undi.Crip(p_Secao), frm_col_undi.Crip(p_Chave), frm_col_undi.Crip(p_Valor));
...
function Tfrm_col_undi.GetValorChaveRegIni(p_SectionName, p_KeyName, p_IniFileName : String) : String;
var
...
Function Tfrm_col_undi.Explode(Texto, Separador : String) : TStrings;
var
    strItem : String;
...
function Tfrm_col_undi.GetRootKey(strRootKey: String): HKEY;
begin
...
Function Tfrm_col_undi.RemoveCaracteresEspeciais(Texto : String) : String;
var I : Integer;
    strAux : String;
Begin
...
function Tfrm_col_undi.GetValorChaveRegEdit(Chave: String): Variant;
...
begin
    try
    Result := '';
    ListaAuxGet := frm_col_undi.Explode(Chave, '\');
    ...
    RegEditGet := TRegistry.Create;
        ...
        Begin
             ...
             begin
               ...
               Result := frm_col_undi.RemoveCaracteresEspeciais(s);
             end
        end;
    finally
    RegEditGet.CloseKey;
...
Function Tfrm_col_undi.Crip(PNome: String): String;
Var
  TamI, TamF: Integer;
...
Function Tfrm_col_undi.DesCrip(PNome: String): String;
Var
  TamI, TamF: Integer;
  SenA, SenM, SenD: String;
Begin
...
procedure Tfrm_col_undi.Log_Historico(strMsg : String);
var
...
Begin
  ...
  Try
    ...
    ValorChaveRegistro := Trim(GetValorChaveRegIni('Coleta','UnidadesDisco',p_path_cacic_ini));
    ...
    If (GetValorChaveRegIni('Configs','IN_COLETA_FORCADA_UNDI',p_path_cacic_ini)='S') or (strXML <> ValorChaveRegistro) Then
     Begin
       SetValorChaveRegIni('Col_Undi','UnidadesDisco', strXML, GetValorChaveRegIni('Configs','P_PATH_COLETAS_INI',p_path_cacic + 'cacic2.ini')+'col_undi.ini');
     end
    else SetValorChaveRegIni('Col_Undi','nada', 'nada', GetValorChaveRegIni('Configs','P_PATH_COLETAS_INI',p_path_cacic + 'cacic2.ini')+'col_undi.ini');

    application.Terminate;
  Except
    SetValorChaveRegIni('Col_Undi','nada', 'nada', GetValorChaveRegIni('Configs','P_PATH_COLETAS_INI',p_path_cacic + 'cacic2.ini')+'col_undi.ini');
    application.Terminate;
  End;
end;

procedure Tfrm_col_undi.FormCreate(Sender: TObject);
